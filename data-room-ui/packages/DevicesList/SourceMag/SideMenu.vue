  ...
  methods: {
    ...
    addOrEditCatalog () {
      this.$refs.form.validate(async (valid) => {
        ...
        if (!this.currentCatalog.id) {
          this.$dataRoomAxios.post('/api/client/group/add', {
            ...this.currentCatalog,
            type: 'resourceCatalog'
          })
            .then((data) => {
          ...
          this.$dataRoomAxios.post('/api/client/grou/update', {
            ...this.currentCatalog,
            type: 'resourceCatalog'
          })
            .then((data) => {
    ...
    catalogDel (catalog) {
      this.$confirm('分组删除后，分组下的资源会被归纳至全部中，确定删除该分组？', '提示', {
        ...
        .then(async () => {
          this.$dataRoomAxios.post(`/api/client/group/delete/${catalog.code}`)
            .then(() => {
              this.$message({
                type: 'success',
                message: '删除成功'
