    ...
    <div class="content">
      <div class="big-screen-list-wrap">
        ...
        >
          ...
          >
            <!-- <div :class="{'big-screen-card-inner': true, 'focus': screen.selected}"> -->
            <div
              :class="{
                'big-screen-card-inner': true,
                'focus': screen.selected || (focus.id == screen.id) || isSelected(screen)
              }"
...
export default {
  ...
  methods: {
    isSelected (screen) {
      return this.selectedList.includes(screen.id)
    },
    CheckAllChange () {
      // console.log(value)
      this.list.forEach(screen => {
        // screen.selected = this.selectAll
        this.chooseImg(screen)
      })
    },
    chooseImg (img) {
      if (this.selectedList.includes(img.id)) {
        const index = this.selectedList.indexOf(img.id)
        if (index > -1) {
          this.selectedList.splice(index, 1)
        }
      } else {
        this.selectedList.push(img.id)
      }
    },
    close () { },
    init () {
    ...
    confirm () {
      ...
      const postData = JSON.stringify(this.selectedList)
      ...
      const hash = urls.split('#')[1]
      const params = new URLSearchParams(hash.split('?')[1])
      const code = params.get('code')
      const url = window.location.origin + '/#/bigscreen/preview?code=' + code

      this.$dataRoomAxios.post('/api/client/SendUrl', {
        device: postData,
        url: url
      }, true).then((r) => {
        if (r.code === 200) {
    ...
    getCoverPicture (url) {
      ...
      return require('../SourceMag/SourceList/images/DLPDevice.svg')
    },
  }
}
</script>
